...
import { ArrowLeft, Trash2, Clock3 } from 'lucide-react-native';
...
import { useHistory, useClearHistory } from '@/services/database/useLibraryQueries';
...
export function HistoryDetailScreen() {
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (filter === 'videos') return historyQuery.data.filter(v => !v.isShort);
    return historyQuery.data.filter(v => v.isShort);
  }, [historyQuery.data, filter]);

  const handlePress = React.useCallback((video: VideoSummary) => {
    if (video.isShort) {
          ...
          if (filter === 'shorts') {
            return (
              <View style={[styles.shortGridItem, index % 2 === 0 ? { paddingRight: 6 } : { paddingLeft: 6 }]}>
                <VideoCard video={item} layout="short" context="history" onPress={handlePress} />
              </View>
            );
          }
          return <VideoCard video={item} layout="compact" context="history" onPress={handlePress} />;
        }}
        ListEmptyComponent={
          !historyQuery.isLoading ? (
            <View style={styles.emptyState}>
              <Clock3 color={colors.textMuted} size={42} style={{ marginBottom: spacing.sm }} />
              <AppText muted>Your {filter} watch history is empty.</AppText>
            </View>
          ) : null
        }
      />
